...
if (!rol) return message.channel.send("**Bir Kick Yetkilisi Rolü Belirtiniz!**").then(x => x.delete({timeout: 5000}));
  
db.set(`kyrol${message.guild.id}`, rol.id)
const embed = new Discord.MessageEmbed()
.setDescription(`Kick Yetkilisi Rolü Başarıyla ${rol} Olarak Ayarlandı!`)
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
